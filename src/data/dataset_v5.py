...
class MiningDataset(Dataset):
    def __init__(self, data_dir, num_points=4096, split='train', aug_config=None, 
        ...
        if use_filtered and filtered_file:
            ...
            if os.path.exists(filtered_path):
                ...
                print(f"✅ Dataset V5 {split}: {len(self.file_list)} bloques FILTRADOS")
            else:
                self.file_list = glob.glob(os.path.join(data_dir, "*.npy"))
        else:
            self.file_list = glob.glob(os.path.join(data_dir, "*.npy"))
            print(f"✅ Dataset V5 {split}: {len(self.file_list)} bloques (sin filtro)")
    ...
    def _apply_runtime_oversampling(self):
        ...
        print(f"🔁 Runtime Oversampling V5: {len(self.file_list)} bloques total (Factor {self.oversample_machinery + 1}x)")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            N = len(data)
            if N >= self.num_points: choices = np.random.choice(N, self.num_points, replace=False)
            else: choices = np.random.choice(N, self.num_points, replace=True)
            data = data[choices, :]
            ...
            if num_cols == 10: # V5
                ...
                xyz = data[:, 0:3]; rgb = None; normals = data[:, 3:6]; labels = data[:, 7].astype(np.int64)
            ...
            if rgb is not None:
                ...
                features = np.hstack([xyz, normals, np.zeros((len(xyz), 1))]) # Dummy
