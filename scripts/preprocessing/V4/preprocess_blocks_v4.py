#!/usr/bin/env python3
"""
Script de Preprocesamiento de Bloques V4 (RGB Integration)
Genera bloques .npy con RGB + Normales + Balance de Clases.

Formato de salida V4 (11 canales): 
[x, y, z, red, green, blue, nx, ny, nz, verticalidad, label]

Cambios vs V3:
- Inclusión de canales RGB normalizados [0.0 - 1.0].
"""
...
DEFAULT_RAW_DIR = "data/raw RGB" # Default actualizado a la nueva carpeta
...
def crop_block(xyz, rgb, features, labels, cx, cy, block_size):
    ...
    Args:
        xyz: (N, 3)
        rgb: (N, 3) Normalized RGB
        features: (N, 4) [nx, ny, nz, v]
        labels: (N,)
    Returns:
        data: (M, 10) [x_rel, y_rel, z_rel, r, g, b, nx, ny, nz, v]
        labels: (M,)
    """
    half = block_size / 2.0
...
def get_hard_negative_crops(xyz, rgb, features, labels, n_needed, block_size, min_points, vert_threshold=0.20):
    ...
    while len(crops) < n_needed and attempts < max_attempts:
        ...
        avg_verticality = np.mean(crop[:, 9]) # Canal 9 es verticalidad (3+3+3)
...
def process_file(filepath, output_dir, config):
    ...
    for i, crop_data in enumerate(all_crops):
        ...
        # Guardar V4: [10 feats] + [label] = 11 canales
    ...
    print(f"   ✅ Guardados: {saved_count['MACHINERY']} MACH | {saved_count['HARD_NEGATIVE']} HARD | {saved_count['EASY_NEGATIVE']} EASY")
    return saved_count


def main():
    parser = argparse.ArgumentParser(description='Preprocesamiento V4 RGB')
    ...
    print(f"🚀 Iniciando Preprocesamiento V4 (RGB) en {args.raw_dir}...")
    print(f"   Bloque: {args.block_size}m | Normal Radius: {args.normal_radius}m")
