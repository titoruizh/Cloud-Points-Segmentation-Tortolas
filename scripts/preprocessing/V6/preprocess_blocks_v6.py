#!/usr/bin/env python3
"""
Script de Preprocesamiento de Bloques V6 (Resolution Sync: 0.25m)
Genera bloques .npy con RGB + Normales (SIN Verticalidad) a partir de nubes de 0.25m.

Base: V5 (Ablation V5 logic).
Input: data/raw RGB/0.25m
...
DEFAULT_RAW_DIR = "data/raw RGB/0.25m"
DEFAULT_BLOCK_SIZE = 10.0
DEFAULT_NORMAL_RADIUS = 2.0
DEFAULT_NORMAL_MAX_NN = 50
DEFAULT_MIN_POINTS = 500 # Bajamos un poco min points porque 0.25m es menos denso
...
def get_machinery_crops(xyz, rgb, features, labels, block_size, min_points, min_machinery_ratio=0.03):
    ...
    if len(mach_xyz) < 20: # Menos exigente con 0.25m
        return []

    adaptive_ratio = min_machinery_ratio * (10.0 / block_size) ** 2
    adaptive_ratio = max(adaptive_ratio, 0.003)
    
    clustering = DBSCAN(eps=3.0, min_samples=10).fit(mach_xyz) # min_samples 10 por menor densidad
...
def process_file(filepath, output_dir, config):
    ...
    if n_machinery < 20: # Ajustado para 0.25m
        return None
    
    print(f"\n📂 Procesando V6: {filename}")
    ...
    for i, crop_data in enumerate(all_crops):
        ...
        # --- ABLATION V5/V6: REMOVER VERTICALIDAD ---
        # Data tiene 10 columnas:
        # [X Y Z R G B Nx Ny Nz V] -> Indices 0..9
        # Queremos guardar solo: [X Y Z R G B Nx Ny Nz] -> Indices 0..8
        
        data_v6 = data[:, :9]  # Excluye columna 9 (Vert)
        
        save_array = np.hstack((data_v6, lbl.reshape(-1, 1)))
        
        out_name = f"{tipo}_{filename}_{i:04d}.npy"
        np.save(os.path.join(output_dir, out_name), save_array.astype(np.float32))
        saved_count[tipo] += 1
    
    print(f"   ✅ Guardados (V6 No-Vert): {saved_count['MACHINERY']} MACH | {saved_count['HARD_NEGATIVE']} HARD | {saved_count['EASY_NEGATIVE']} EASY")
    return saved_count


def main():
    parser = argparse.ArgumentParser(description='Preprocesamiento V6 (0.25m)')
    ...
    print(f"🚀 Iniciando Preprocesamiento V6 (0.25m) en {args.raw_dir}...")
