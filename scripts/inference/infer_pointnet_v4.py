...
def compute_features_global_v4(xyz, rgb_raw, radius=2.5):
    ...
    if rgb_raw is not None:
        # Open3D espera RGB en float [0,1]
...
class GridDatasetV4(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # --- NORMALIZACIÓN GEOMÉTRICA (V2 Match) ---
        ix = key // 100000
        iy = key % 100000
        ...
        # [x_norm, y_norm, z_norm, r, g, b, nx, ny, nz, v]
...
def run_inference(args):
    ...
    print(f"🚀 Iniciando Inferencia V4 (RGB) Grid-Based: {args.input_file}")
    ...
    if match:
        ...
        radius = 3.5 # Default V4 baseline
        print(f"⚠️ No se detectó radio. Usando default V4: {radius}m")
    ...
    model = PointNet2(d_in=10, num_classes=2, base_radius=radius).to(device)
    ...
    with torch.no_grad():
        for features, batch_indices_list in tqdm(dataloader):
            ...
            # XYZ para PointNet2 (argumento explícito)
            ...
            # --- FIX HEURÍSTICO V4 ---
            # features[:, :, 9] es verticalidad (index 9)
    ...
    # Predicciones binarias (Threshold)
    ...
    print(f"💾 Guardando V4 LAS: {args.output_file}")
    ...
    new_las.y = xyz[:, 1]
    new_las.z = xyz[:, 2]
    # Guardar color original (denormalized if needed? No, laspy expects uint16, original las had it)
    # Mejor usar el 'rgb' limpio si modificamos los puntos
    if rgb is not None:
...
if __name__ == "__main__":
    ...
    if args.output_file is None:
        ...
        args.output_file = os.path.join(output_dir, f"{filename_no_ext}_POINTNET_V4_RGB.laz")
