...
def compute_features_global_v5(xyz, rgb_raw, radius=2.5):
    """
    Computa normales. Integra RGB normalizado.
    V5: NO CALCULA VERTICALIDAD.
    Output: XYZ, RGB, Normals (9 canales)
    """
    print("🧹 [V5] Limpiando nube de puntos (Duplicados, NaNs)...")
    ...
    print(f"🔍 [V5] Calculando normales globales (r={radius}m)...")
    ...
    # V5: NO Computamos verticalidad
    return xyz_clean, rgb_clean, normals

class GridDatasetV5(Dataset):
    def __init__(self, full_data, grid_dict, num_points, min_coord, block_size):
        """
        full_data: [N, 9] -> [x, y, z, r, g, b, nx, ny, nz]
        """
    ...
    def __getitem__(self, idx):
        ...
        # [x_norm, y_norm, z_norm, r, g, b, nx, ny, nz]
...
def run_inference(args):
    ...
    print(f"🚀 Iniciando Inferencia V5 (NoVert) Grid-Based: {args.input_file}")
    ...
    if hasattr(las, 'red') and hasattr(las, 'green') and hasattr(las, 'blue'):
        print("🌈 Color RGB detectado y normalizado.")
        ...
        print("⚠️ NO se detectó canal RGB. Rellenando con gris...")
    ...
    print(f"📡 Radio Base: {radius}m")

    # V5: Solo XYZ, RGB, Normals
    xyz, rgb, normals = compute_features_global_v5(xyz_raw, rgb_raw, radius=radius)
    
    # Concatenar: [x, y, z, r, g, b, nx, ny, nz] (9 canales)
    full_data = np.hstack([xyz, rgb, normals]) 
    ...
    # 4. DataLoader
    dataset = GridDatasetV5(full_data, grid_dict, args.num_points, min_coord, args.block_size)
    dataloader = DataLoader(dataset, batch_size=args.batch_size, num_workers=4, shuffle=False)
    
    # 5. Inferencia
    print(f"🏗️ Cargando modelo V5 (d_in=9): {args.checkpoint}")
    # NOTA: d_in=9 para V5
    ...
    with torch.no_grad():
        for features, batch_indices_list in tqdm(dataloader):
            features = features.to(device) # [B, N, 9]
            ...
            # SIN FILTRO HEURÍSTICO DE VERTICALIDAD (Ablation V5)
    ...
    # DBSCAN Clean Up
    print("🧹 Filtrando ruido con DBSCAN...")
    ...
    if len(pred_indices) > 0:
        ...
        # Params calibrados V4 (reutilizados)
    ...
    print(f"💾 Guardando V5 LAS: {args.output_file}")
...
if __name__ == "__main__":
    ...
    if args.output_file is None:
        ...
        output_dir = "data/predictions_v5"
        os.makedirs(output_dir, exist_ok=True)
        args.output_file = os.path.join(output_dir, f"{filename_no_ext}_POINTNET_V5_NOVERT.laz")
